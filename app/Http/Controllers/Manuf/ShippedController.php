...
class ShippedController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('manuf.shipped.index', compact('state_id') );
    }
    ...
    {
        $user = Auth::user();
        if( $user_shipped->user_id != $user->id ){ return abort(404); }

        # ガチャタイトル
        $item = ManufPurchaseItem::where('shipped_id',$user_shipped->id)
        ...
        return view('manuf.shipped.show',compact(
            'item','machine','gacha_title',
            'history','shipped_fee' ,'sub_total_fee','total_fee',     
            'user_shipped','shipped_point',
            'user_address','user_prizes','shipped_prizes'
        ));
    }
