...
/*
| =============================================
|  ストアーAdmin　ポイント売上レポート(改正版) コントローラー
| =============================================
*/
class AdminPointSalesReportController extends Controller
{
    ...
    */
    public function api_index(Request $request)
    {
        ...
        $period = CarbonPeriod::create($start_day, $last_day);// 日付期間を作成
        foreach ($period as $day) {
            $data_list['r_daily_array'][] = route('admin.point_sales_report.daily',$day->format('Y-m-d'));
        }
        ...
        # ガチャ回転数(gacha_played_count)
        list($total_gacha_played_count, $data_list['gacha_played_count'] )
        = self::aggGachaPlayedCount( $start_day, $last_day);
        ...
        $totals = [
            ...
            'gacha_played_count'     => ['value'=> $total_gacha_played_count, 'label'=> 'ガチャ回転数'], //ガチャ回転数
        ...
        # ルーティング
        $r_api_visiters = route('admin.api.point_sales_report.visiters');//API 顧客一覧
        // $r_api_products = route('admin.api.point_sales_report.products');//API 販売ポイント一覧
        ...
        return response()->json( compact(
            ...
            'data_list','totals','select_day_types',
            'inputs',
            'r_api_visiters',
        ) );
    }
        ...
        */
        public static function aggVisitersCount( $start_day, $last_day)
        {
            $days = [];
            $total = 0;
        ...
        */
        public static function aggRepraterCount( $start_day, $last_day)
        {
            $days = [];
            $total = 0;
            ...
            # 総合リピーター数
            $total = User::whereHas('point_histories', function ($query) use ($start_day, $last_day) {
        ...
        */
        public static function aggPaymentCount( $start_day, $last_day)
        {
            $days = [];
            $total = 0;
        ...
        /**
         * [メソッド] ガチャ回転数(sales_product_count)
        */
        public static function aggGachaPlayedCount( $start_day, $last_day)
        {
            $days = [];
            $total = 0;
            ...
            $period = CarbonPeriod::create($start_day, $last_day);// 日付期間を作成
            foreach ($period as $day) {
                $daily_val = Daily::getGachaPlayedCount($day);
                $days[]    = $daily_val;
                $total    += $daily_val;
            }
