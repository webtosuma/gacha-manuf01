...
<label class="d-block mb-4">
    ...
    <!--error message-->
    @if ( $errors->has('enactmented_at') )
        <div class="text-danger"> {{$errors->first('enactmented_at')}} </div>
    @endif

    <div style="max-width:300px">
        <input name="enactmented_at"
        type="date" class="form-control"
        value="{{ $text->enactmented_at }}"
        min="">
    </div>
